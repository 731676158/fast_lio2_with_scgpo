...
void removeNaNFromPointCloud (const pcl::PointCloud<velodyne_ros::Point> &cloud_in,
                              pcl::PointCloud<velodyne_ros::Point> &cloud_out,
  ...
  else
  {
    ...
    for (std::size_t i = 0; i < cloud_in.points.size (); ++i)
    {
      if (!std::isfinite (cloud_in.points[i].x) ||
        ...
        continue;
      if (isnan(cloud_in.points[i].time) ||
           isnan(cloud_in.points[i].ring) ||
           isnan(cloud_in.points[i].intensity))
           continue;
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    pcl::fromROSMsg(*msg, pl_orig);
    std::vector<int> mid;
    removeNaNFromPointCloud(pl_orig, pl_orig, mid);
...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  for(uint i=head; i<plsize2; i++)
  {
    ...
    if (i_nex >= types.size()) types.resize(i_nex + 1);
